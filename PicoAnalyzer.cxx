...
// pt SetA, cent SetA
  ...
  for(Long64_t iEvent=0; iEvent<events2read; iEvent++) //events2read
  {
    ...
    for(int EventTypeId=0;EventTypeId<_nEventTypeBins;EventTypeId++){
      ...
      if(QrawEastSide[0][EventTypeId][0] || QrawEastSide[0][EventTypeId][1] )
      {
        // Double_t QrawXtemp = (Double_t)QrawEastSide[0][EventTypeId][0];
        // Double_t QrawYtemp = (Double_t)QrawEastSide[0][EventTypeId][1];
        PsiEastRaw[0][EventTypeId] = (Double_t)GetPsi(QrawEastSide[0][EventTypeId][0],QrawEastSide[0][EventTypeId][1],EpOrder);
        if(PsiEastRaw[0][EventTypeId]!=-999.0){
          hist2_Epd_east_Qy_Qx_raw_ini[0][EventTypeId]->Fill(QrawEastSide[0][EventTypeId][0],QrawEastSide[0][EventTypeId][1]);
          hist_Epd_east_psi_raw_ini[0][EventTypeId]->Fill(PsiEastRaw[0][EventTypeId]);
          // hist_Epd_east_psi_Weighted_ini[EventTypeId]->Fill(PsiEastPhiWeighted[EventTypeId]);
        } else {
          cout << "PsiEastRaw[0]  " << EventTypeId << " = " << PsiEastRaw[0][EventTypeId]<<endl;
          cout << "Qx raw  " << EventTypeId << " = " << QrawEastSide[0][EventTypeId][0]<<endl;
          cout << "Qy raw  " << EventTypeId << " = " << QrawEastSide[0][EventTypeId][1]<<endl;
        }
        ...
        PsiEastRecenter[0][EventTypeId] = GetPsi(QrecenterEastSide[0][EventTypeId][0],QrecenterEastSide[0][EventTypeId][1],EpOrder);
        if(PsiEastRaw[0][EventTypeId]!=-999.0){
          hist2_Epd_east_Qy_Qx_rec_ini[0][EventTypeId]->Fill(QrecenterEastSide[0][EventTypeId][0],QrecenterEastSide[0][EventTypeId][1]);
          hist_Epd_east_psi_recenter_ini[0][EventTypeId]->Fill(PsiEastRecenter[0][EventTypeId]);
          // cout << "Psi_raw = " << PsiEastRaw[0][EventTypeId] << endl;
          // cout << "Psi_rec = " << PsiEastRecenter[0][EventTypeId] << endl;
          // hist_Epd_east_psi_Weighted_ini[EventTypeId]->Fill(PsiEastPhiWeighted[EventTypeId]);
          // -------------------- "recenter correction histograms Output" ----------------
          // -------------------- "calculate recenter histograms for a future run" ----------------
          // Fill the recenter plots for next run
          mEpdRecenterOutput[EventTypeId]->Fill(1,centrality,QrawEastSide[0][EventTypeId][0]);
          mEpdRecenterOutput[EventTypeId]->Fill(2,centrality,QrawEastSide[0][EventTypeId][1]);
        }
        // cout << "QrawEastSide[0] Qx"<<EventTypeId <<" = " << QrawEastSide[0][EventTypeId][0] << endl;
        // cout << "QrawEastSide[0] Qy"<< EventTypeId <<" = " << QrawEastSide[0][EventTypeId][1] << endl;
        // mEpdRecenterOutput[EventTypeId]->Fill(1,centrality,QrawEastSide[0][EventTypeId][0]);
        // mEpdRecenterOutput[EventTypeId]->Fill(2,centrality,QrawEastSide[0][EventTypeId][1]);
      }
    }
    ...
    for(int EventTypeId=0; EventTypeId<_nEventTypeBins; EventTypeId++){ //etaRange {-5.1,-4.2,-3.28,-2.87,-2.60}
        PsiEastShifted[0][EventTypeId] = PsiEastRecenter[0][EventTypeId]; // use raw EP rather than Phi weighing EP
        if(PsiEastShifted[0][EventTypeId]==-999.0) continue;
        if (mEpdShiftInput_sin[EventTypeId] != 0 && mEpdShiftInput_cos[EventTypeId]!= 0){
          for (int i=1; i<=_EpTermsMaxIni; i++){
            ...
            double sinAve = mEpdShiftInput_sin[EventTypeId]->GetBinContent(i,centrality);
	         ...
	         double AngleWrapAround = 2.0*TMath::Pi()/(double)EpOrder;
  	        if (PsiEastShifted[0][EventTypeId]<0) PsiEastShifted[0][EventTypeId] += AngleWrapAround;
  	         else if (PsiEastShifted[0][EventTypeId]>AngleWrapAround) PsiEastShifted[0][EventTypeId] -= AngleWrapAround;
        }
        hist_Epd_east_psi_Shifted_ini[0][EventTypeId]->Fill(PsiEastShifted[0][EventTypeId]);
      }
    ...
    for (int i=1; i<=_EpTermsMaxIni; i++){
      for(int EventTypeId=0; EventTypeId<_nEventTypeBins; EventTypeId++){//etaRange {-5.1,-4.2,-3.28,-2.87,-2.60}
        ...
        mEpdShiftOutput_sin[EventTypeId]->Fill(i,centrality,sin(tmp*PsiEastRecenter[0][EventTypeId]));// use raw EP rather than Phi weighing EP
        mEpdShiftOutput_cos[EventTypeId]->Fill(i,centrality,cos(tmp*PsiEastRecenter[0][EventTypeId]));// use raw EP rather than Phi weighing EP
      }
    }
    ...
    for(unsigned int i=0; i<vGoodTracks.size();i++){
      ...
      for(int EventTypeId_tpc=0;EventTypeId_tpc<_nEventTypeBins_tpc;EventTypeId_tpc++){
        ...
        if(EpOrder == 1){ // \psi_1^{TPC}
          ...
          double Cosine = cos(phi*(double)EpOrder);
          double Sine   = sin(phi*(double)EpOrder);
          QrawTpcAll[0][EventTypeId_tpc][0] += etaWeight * etaTrkWeight /*rapWeight*/ * Cosine;
          QrawTpcAll[0][EventTypeId_tpc][1] += etaWeight * etaTrkWeight /*rapWeight*/ * Sine;
        } else { // \psi_2^{TPC}
          if(etaWeight>0.0) NTpcAll[EventTypeId_tpc]++;
          double Cosine = cos(phi*(double)EpOrder);
          double Sine   = sin(phi*(double)EpOrder);
          QrawTpcAll[0][EventTypeId_tpc][0] += etaWeight * pt * Cosine;
          QrawTpcAll[0][EventTypeId_tpc][1] += etaWeight * pt * Sine;
        }
      }
    ...
    for(int EventTypeId_tpc=0;EventTypeId_tpc<_nEventTypeBins_tpc;EventTypeId_tpc++){
      ...
      if(QrawTpcAll[0][EventTypeId_tpc][0] || QrawTpcAll[0][EventTypeId_tpc][1] ){ // Qx, Qy cannot be 0 at the same time
        ...
        if(mTpcRecenterInput[EventTypeId_tpc]==0){
          ...
          QrecenterTpcAll[0][EventTypeId_tpc][0] = QrawTpcAll[0][EventTypeId_tpc][0];
          QrecenterTpcAll[0][EventTypeId_tpc][1] = QrawTpcAll[0][EventTypeId_tpc][1];
        } else {
          QrecenterTpcAll[0][EventTypeId_tpc][0] = QrawTpcAll[0][EventTypeId_tpc][0] - mTpcRecenterInput[EventTypeId_tpc]->GetBinContent(1,centrality);
    ...
    for(int EventTypeId_tpc=0;EventTypeId_tpc<_nEventTypeBins_tpc;EventTypeId_tpc++){
      PsiTpcAllShifted[0][EventTypeId_tpc] = PsiTpcAllRecenter[0][EventTypeId_tpc];
      if(PsiTpcAllShifted[0][EventTypeId_tpc]==-999.0) continue; // Bad PsiTpcAllRecenter[0]
      if (mTpcShiftInput_sin[EventTypeId_tpc] != 0 && mTpcShiftInput_cos[EventTypeId_tpc]!= 0){
        for (int i=1; i<=_EpTermsMaxIni; i++){
            ...
            PsiTpcAllShifted[0][EventTypeId_tpc] +=
              2.0*(cosAve*sin(tmp*PsiTpcAllRecenter[0][EventTypeId_tpc]) - sinAve*cos(tmp*PsiTpcAllRecenter[0][EventTypeId_tpc]))/tmp;
        }
           double AngleWrapAround = 2.0*TMath::Pi()/(double)EpOrder;
    ...
    for(int EventTypeId_tpc=0; EventTypeId_tpc<_nEventTypeBins_tpc; EventTypeId_tpc++){
      for (int i=1; i<=_EpTermsMaxIni; i++){ // TPC shifted Output
        double tmp = (double)(EpOrder*i);
        if(PsiTpcAllRecenter[0][EventTypeId_tpc]==-999.0) break;
        mTpcShiftOutput_sin[EventTypeId_tpc]->Fill(i,centrality,sin(tmp*PsiTpcAllRecenter[0][EventTypeId_tpc]));
        mTpcShiftOutput_cos[EventTypeId_tpc]->Fill(i,centrality,cos(tmp*PsiTpcAllRecenter[0][EventTypeId_tpc]));
      }
    }
  ...
  for(int EventTypeId=0; EventTypeId<_nEventTypeBins; EventTypeId++){
    hist2_Epd_east_Qy_Qx_raw_ini[0][EventTypeId]->GetXaxis()->SetTitle("Q_x^{raw EPD east}_{1} ");
    hist2_Epd_east_Qy_Qx_raw_ini[0][EventTypeId]->GetYaxis()->SetTitle("Q_y^{raw EPD east}_{1} ");
    hist2_Epd_east_Qy_Qx_rec_ini[0][EventTypeId]->GetXaxis()->SetTitle("Q_x^{rec EPD east}_{1} ");
    hist2_Epd_east_Qy_Qx_rec_ini[0][EventTypeId]->GetYaxis()->SetTitle("Q_y^{rec EPD east}_{1} ");
    hist_Epd_east_psi_raw_ini[0][EventTypeId]->GetXaxis()->SetTitle("#psi^{EPD east}_{1} [Radian]");
    hist_Epd_east_psi_raw_ini[0][EventTypeId]->GetYaxis()->SetTitle("# of events");
    ...
    hist_Epd_east_psi_Shifted_ini[0][EventTypeId]->GetXaxis()->SetTitle("#psi^{EPD east}_{1} [Radian]");
    hist_Epd_east_psi_Shifted_ini[0][EventTypeId]->GetYaxis()->SetTitle("# of events");
  }
